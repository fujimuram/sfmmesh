#結び目探索(4角形網):targetは探索範囲に戻さない
#端点から10近傍に対して補正線を引き、なす角が最も大きくなるベクトル1つを採用する(point_array2でループ)
...
work_dir = "thinning4_2"

pcd = o3d.io.read_point_cloud("./" + work_dir + "/thin_avg.ply")
point_array = np.asarray(pcd.points)
point_array = sorted(point_array, key=lambda x:(x[1], x[0]))#y座標で昇順に並べてからx軸で昇順に並べる
...
for n, k in enumerate(point_array2):
    ...
    if(points.count(k) == 1):
        ...
        distance, index = tree.query(k,10)#(復元対象のスケールに合わせて変更が必要)
        print("---------------------------------------------------------------------------------------------")
        print(" target: #", n, k, "\n", "distance:", distance, "\n", "index:", index, "\n")
        ...
        for a in range(len(index)):
            ...
            deg_list.append(deg)
            if(points.count(point_array2[index[a]].tolist()) == 3):
...
#重複削除と網の交点抽出
for k in points:
    if(points.count(k) == 4 and not(k in new_points)):
        new_points.append(k)
