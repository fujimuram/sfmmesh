...
IMAGE_PATH = "./images/test.jpg" # 読み込む画像
...
def main():
    ...
    cv2.imwrite("./images/outLine.png", outLineImage)    # ファイル保存

    hough  = hough_lines(image, outLineImage)     # ハフ変換による直線抽出
    cv2.imwrite("./images/result_hough.png", image)    # ファイル保存

    houghP = hough_lines_p(image2, outLineImage)   # 確率的ハフ変換による直線抽出
    cv2.imwrite("./images/result_houghP.png", image2)  # ファイル保存
