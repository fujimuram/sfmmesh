...
pcd = o3d.io.read_point_cloud("./voxel_reduction_ply/scene_dense_reduction_voxel_0.001000_4_2_half.ply")
#pcd = o3d.io.read_point_cloud("./voxel_reduction_ply/thin_avg.ply")
...
while len(point_array) != 0:
    ...
    drop_list = tree.query_ball_point(point_array[0], r=2)#rは復元対象のスケールに合わせて変更が必要
    #近傍点がないときはターゲットの点1つだけがdrop_listに入る
    
    if(len(drop_list) < 4):#ターゲットの近傍点数に対するしきい値
        for j in drop_list:
