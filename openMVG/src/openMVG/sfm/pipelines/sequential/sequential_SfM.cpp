...
bool SequentialSfMReconstructionEngine::ChooseInitialPair(Pair & initialPairIndex) const
{
  ...
  else
  {
    ...
    for (size_t i = 0; i < std::min((size_t)10, vec_NbMatchesPerPair.size()); ++i) {
      ...
      std::cout << "(" << iter->first.first << "," << iter->first.second <<")\t\t" << iter->second.size() << " matches" << std::endl;
	...
	std::cout << "Choose pair is" << "(" << initialPairIndex.first << "," << initialPairIndex.second << ")\n" << std::endl;
  ...
  std::cout << "\nPutative starting pair is: (" << initialPairIndex.first << "," << initialPairIndex.second << ")" << std::endl;
